    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        doc
          ...
          .text(
            `${invoice.items?.[0]?.currency ? invoice.items?.[0]?.currency : '$'}${invoice.totalAmount}`,
            480,
        ...
        for (i = 0; i < items.length; i++) {
          ...
          generateTableRow({
            ...
            price: `${invoice.items?.[0]?.currency ? invoice.items?.[0]?.currency : '$'}${items[i]?.rate}`,
            amount: `${invoice.items?.[0]?.currency ? invoice.items?.[0]?.currency : '$'}${items[i]?.amount}`,
          });
        ...
        generateTableRow({
          ...
          quantity: '',
          price: 'Subtotal',
          amount: `${invoice.items?.[0]?.currency ? invoice.items?.[0]?.currency : '$'}${invoice.subtotal}`,
        });
        ...
        generateTableRow({
          ...
          quantity: '',
          price: 'Tax amount',
          amount: `${invoice.items?.[0]?.currency ? invoice.items?.[0]?.currency : '$'}${invoice.taxAmount ?? 0}`,
        });
        ...
        generateTableRow({
          ...
          quantity: '',
          price: 'Total',
          amount: ` ${invoice.items?.[0]?.currency ? invoice.items?.[0]?.currency : '$'}${invoice?.totalAmount}`,
        });
